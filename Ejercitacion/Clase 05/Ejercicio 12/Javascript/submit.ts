function agregarCiudad() {
    ...
    let latitud : number = Number((<HTMLInputElement>document.getElementById("lat")).value);
    let longitud : number = Number((<HTMLInputElement>document.getElementById("long")).value);
    let json : any = {"_id" : id , "name" : nombre , "country" : pais , "coord" : {"lon" : longitud , "lat" : latitud}}
    xhttp.onreadystatechange = () => {
        if(  xhttp.status == 404) {
            ...
            document.getElementById("msg").innerText = "EXITO";
        }
    }
    xhttp.open("POST", "./PHP/administrarCiudades.php",true);
    xhttp.setRequestHeader("content-type","application/x-www-form-urlencoded");
    xhttp.send(`option=agregarCiudad&json=${JSON.stringify(json)}`);
}

function eliminarCiudad(id : string) {
    let xhttp: XMLHttpRequest = new XMLHttpRequest();
    xhttp.open("POST", "./PHP/administrarCiudades.php",true);
    xhttp.setRequestHeader("content-type","application/x-www-form-urlencoded");
    xhttp.send(`option=quitarCiudad&id=${id}`);
}

function traerCiudades() {
    let xhttp: XMLHttpRequest = new XMLHttpRequest();
    let tabla : HTMLTableElement = document.createElement("table");
    let titulos : string[] = ["Id","Nombre","País","Coordenadas","Action"];
    tabla.createTHead();
    tabla.border = "1";
    for(let i = 0 ; i < 6 ; i++) {
    ...
    xhttp.onreadystatechange = () => {
        if (xhttp.readyState == 4 && xhttp.status == 200) {
            ...
            for(let i = 0 ; i < json.length ; i++) {
                ...
                let coord : string = `longitud ${parseFloat(json[i].coord.lon).toFixed(2)} -\
                 latitud ${parseFloat(json[i].coord.lat).toFixed(2)} `;
                tabla.tBodies[0].rows[i].insertCell().textContent = coord;
                tabla.tBodies[0].rows[i].insertCell().innerHTML = '<a href="#home" onclick="eliminarCiudad(\''+json[i]._id+'\')" >Eliminar</a>';
            }
...
function convert(abrev : string ) : string  {
    let result : string = "";
    switch(abrev) {
        case "AD":
            result = "Andorra"; 
